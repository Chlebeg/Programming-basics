...
void reserve(Vector *vector, size_t new_capacity) {
    if (new_capacity > vector->capacity) {
        realloc(vector->data, new_capacity * vector->element_size);
    }
}
...
void resize(Vector *vector, size_t new_size) {
    if (new_size < vector->size) {
        ...
        for( size_t size; size < new_size; size++) {
            memcpy(vector->data + (int)vector->size * vector->element_size, &a, vector->element_size);
        }
        vector->size = new_size;
    }
}
...
void push_back(Vector *vector, void *value) {
    if (vector->size >= vector->capacity) {
        reserve(vector, vector->capacity ? 2 * vector->capacity : 2);
    }
    memcpy(vector->data + (int)vector->size++ * vector->element_size, value, vector->element_size);
    vector->size++;
}
...
void insert(Vector *vector, int index, void *value) {
    if (index >= 0 && index <= vector->size) {
        ...
        memmove((char *) vector->data + index * vector->element_size,
                (char *) vector->data + (index + 1) * vector->element_size, vector->size - index);
        vector->size++;
    }
}
...
void print_vector_person(Vector *vector) {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        printf("First name: %s, Last name %s, Age %d\n", element->first_name, element->last_name, element->age);
    }
}
...
int is_vowel(void *value) {
    if (*(char *) value == 'a' || *(char *) value == 'e' || *(char *) value == 'i' || *(char *) value == 'o' ||
        *(char *) value == 'u') return 1;
    else return 0;
}
